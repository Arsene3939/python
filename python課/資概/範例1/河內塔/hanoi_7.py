...
disks = 7
print("You hav ", disks, " rings to move")
maxradius=3.5
thick=.5
spacing=maxradius+thick + 1
leftpole=cylinder(pos=(-2*spacing,-3,0), radius=0.3, axis=(0,disks+1,0))
leftpole.color=(.5,.5,.5)
midpole=cylinder(pos=(0,-3,0), radius=0.3, axis=(0,disks+1,0))
midpole.color=(.5,.5,.5)
rightpole=cylinder(pos=(2*spacing,-3,0), radius=0.3, axis=(0,disks+1,0))
floor=box(pos=(0,-3.5,0), size=(28,.99,5),
          color=(1.0,0.5,0), material=materials.wood)

poles=[leftpole,midpole,rightpole]
rings=[]
hues=[(1,0,0), (1,1,0), (0,1,0), (.5,.5,1), (1,0,1),(0,0,1),(0,1,1)]

for y in arange(-2, disks-2,1):
    rings.append(ring(pos=(poles[0].x,y,0), radius=.5*(disks-2-y), color=hues[y+2],
        ...
        if m.drag:  # identify pole clicked on
            ...
            if len(stack[pole1])>0:
                ...
                if len(stack[pole2])>0:      # stack not empty
                    ...
                    stack[pole2].append(select)
                    moves=moves+1
        if len(stack[2])==disks:    # task completed?
