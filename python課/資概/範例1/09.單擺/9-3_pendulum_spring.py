...
"""
# 產生動畫視窗、天花板、小球、彈簧
scene = display(title = "Pendulum with Spring", width = 600, height = 600, x = 0, y = 0, background = vector(0, 0.6, 0.6))
...
ball.v = vector(0, 0, 0)
spring = helix(pos = vector(0, L/2, 0), axis = ball.pos - vector(0, L/2, 0), radius = 0.6*size,coils=15, thickness = 0.3*size, color = color.yellow)
# 產生表示速度的箭頭
...
# 繪圖部分
gd1 = gdisplay(title = "position", width = 600, height = 450, x = 0, y = 600, xtitle = "t(s)", \
...
gd2 = gdisplay(title = "velocity", width = 600, height = 450, x = 0, y = 600, xtitle = "t(s)", \
            ytitle = "blue: v(m/s), red: vx(m/s), green: vy(m/s)")
v_t = gcurve(graph = gd1, color = color.blue)
vx_t = gcurve(graph = gd1, color = color.red)
vy_t = gcurve(graph = gd1, color = color.green)
gd3 = gdisplay(title = "acceleration", width = 600, height = 450, x = 0, y = 600, xtitle = "t(s)", \
