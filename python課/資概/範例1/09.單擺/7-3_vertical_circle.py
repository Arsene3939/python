...
v0 = 0 #1*sqrt(g*R)      # 小球初速, 1 ~ 7 sqrt(g*R)
...
"""
scene = display(title = "Vertical Circle", width = 600, height = 600, x = 0, y = 0, background = color.black)
ball = sphere(pos = vector(R*0.707,-R*0.707, 0), radius = size, color = color.red, make_trail = True, retain = 100)
ball.v = vector(0, 0, 0)
...
gd = gdisplay(title = "plot", width = 600, height = 450, x = 0, y = 600, xtitle = "t(s)", ytitle = "green: v(m/s), red: at(m/s^2), blue: an(m/s^2)")
...
# 更新 v-t, at-t, an-t 圖
    vt_plot.plot(pos =(t, ball.v.mag))
    at_plot.plot(pos =(t, at.mag))
    an_plot.plot(pos =(t, an.mag))
# 更新時間
    t += dt
