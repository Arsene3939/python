...
floor = box(pos = vector(0, -(size+0.1)/2, 0), length = 2*L0, height = 0.1, width = R, color = color.blue)
wall = box(pos = vector(-L0, 0, 0), length = 0.1, height = size, width = R, color = color.cyan)
block = box(pos = vector(R*0+2+size/2, 0, 0), length = size, height = size, width = size, color = color.yellow)
