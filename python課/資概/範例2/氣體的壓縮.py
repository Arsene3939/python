...
wall = box(pos=(-L,0,0), length=0.05*L, height=2*L, width=2*L, color=color.black)
wall.v = vector(0.05*vrms,0,0)
wall.m = m*1000
...
while True:
    ...
    for atom in atoms:        #簡查所有原子是否撞牆  
        ...
        if abs(atom.pos.x-wall.pos.x) < size+wall.length/2 :
            ...
            momentum += m * (abs(temv)+abs(atom.v))
            wall.v = vector(0.05*vrms,0,0)
    wall.pos += wall.v*dt
#    if int(t/dt)%200 ==0: print 'pressure = %6.0f Pa ' % (momentum / 6 / (2*L)**2 / t) #壓力=總動量變化/6面牆/每面牆面積/時間
    if int(t/dt)%200 ==0:
         print 'pressure = %6.0f Pa ' % (momentum / ( 2*((2*L)**2) + 4*((L-wall.pos[0])*2*L) ) / t)
#         print '%0.20f K ' % ( (momentum / ( 2*((2*L)**2) + 4*((L-wall.pos[0])*2*L) ) / t)*(L-wall.pos[0])*(2*L)**2 )
    if wall.pos[0] > 0.7*L:
        break
