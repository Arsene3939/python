...
scene = display(title='dipole', height=1000, width=1000, range=3.5, auto_scale=False, background=(0.4,0.7,0.7))
scene2 = display(title='quadrupole',x=1000, y=0, height=1000, width=1000, range=3.5, auto_scale=False, background=(0.4,0.7,0.7))
...
size1 = 0.1 #m
c1 = []
c1.append(sphere(pos=vector(1,0,0), radius=size1, color=(1,0,0))) #c1[0]
c1.append(sphere(pos=vector(-1,0,0), radius=size1, color=(0,0,1))) #c1[1]
...
for phi_num in range(0,phi_div):
    for theta_num in range(1-theta_div,theta_div):
        ...
        ball1.append(sphere(pos=vector(size1*sin(theta), size1*cos(theta)*cos(phi), size1*cos(theta)*sin(phi))+c1[0].pos, \
                            radius=0.01, color=(1,1,0), make_trail=True, v=vector(0,0,0), acc=vector(0,0,0)))
...
size2 = 0.1 #m
c2 = []
c2.append(sphere(pos=vector(1,0,0), radius=size2, color=(1,0,0))) #c2[0]
c2.append(sphere(pos=vector(-1,0,0), radius=size2, color=(1,0,0))) #c2[1]
